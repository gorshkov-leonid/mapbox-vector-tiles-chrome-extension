...
var JSONViewer = (function(document) {
	...
	function JSONViewer() {
		...
		this._json_container = document.createElement("pre");

		const linkBar = document.createElement('div');
		linkBar.classList.add('link-bar');
		const expandLink = _createLink('Expand all');
		expandLink.addEventListener('click', () => {
			this._json_container.querySelectorAll('.list-link.collapsed').forEach((el) => {
				el.click();
			});
		});
		const collapseLink = _createLink('Collapse all');
		collapseLink.addEventListener('click', () => {
			this._json_container.querySelectorAll('.list-link:not(.collapsed)').forEach((el) => {
				el.click();
			});
		});
		linkBar.appendChild(expandLink);
		linkBar.appendChild(collapseLink);
		this._dom_container.appendChild(linkBar);

		this._dom_container.appendChild(this._json_container);
	};
	...
	JSONViewer.prototype.showJSON = function(jsonValue, inputMaxLvl, inputColAt) {
		...
		this._json_container.innerHTML = "";
		walkJSONTree(this._json_container, jsonValue, maxLvl, colAt, 0);
	};
	...
	function walkJSONTree(outputParent, value, maxLvl, colAt, lvl) {
		...
		if (typeof realValue === "object" && realValue !== null && !isDate) {
			...
			if (lvl === 0) {
				...
				// hide/show
				var rootLink = _createLink(isArray ? "[" : "{", 'list-link');
				...
				items.forEach(function(key, ind) {
					...
					if (typeof item === "object") {
						...
						// array & object
							...
							if (!itemLen) {
							...
							else if (itemIsArray && ((itemLen === 1 && typeof item[0] !== 'object') || (itemLen === 2 && typeof item[0] === 'number' && typeof item[1] === 'number'))) {
								// single-element array, or 2-number array - just show it inline
								var itemTitle = (typeof key === 'string' ? key + ': ' : '') + '[ ';
								li.appendChild(document.createTextNode(itemTitle));
								li.appendChild(createSimpleViewOf(item[0]));
								if (itemLen === 2) {
									li.appendChild(document.createTextNode(', '));
									li.appendChild(createSimpleViewOf(item[1]));
								}
								li.appendChild(document.createTextNode(' ]'));
							}
							else {
								...
								var itemLink = _createLink(itemTitle, 'list-link');
	 ...
	 * @return {Element}
	 */
	function _createLink(title, cssClass) {
		var linkEl = document.createElement("a");

		if (cssClass) {
			linkEl.classList.add(cssClass);
		}
